...
To this end, it would be beneficial to put custom DLL files in the provided location, and validate which ones are successfully loaded. In the case of `SYSTEMROOT` and `WINDIR`, which normally resolve to `C:\Windows`, this would mean over 20,000 'custom' DLL files would have to be compiled for the test to be as complete as it can be. Compiling custom DLLs is not as straight-forward as it may sound [[6]], let alone when several thousand DLLs have to be compiled. 
...
Supporting code, which can be used to compile large amounts of DLLs for the purpose of DLL Hijacking, leveraging open-source tooling and using 'DLL export proxying' and 'DLL resource cloning' to maximise compatibility with the tested executables, can be found on GitHub [[10]]. A more thorough and technical explanation of the approach taken is provided there as well. 
